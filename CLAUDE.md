...
- **Auth persistiert nur unter `%LOCALAPPDATA%\agentbox\auth\<agent>\`**, nie in
  ...
  und nie in der Distro (ephemer). Bei Claude Code speichert das plaintext-
  Backend OAuth-Tokens in `~/.claude/.credentials.json` (via `writeFileSync` +
  `chmod 0o600`) — die Datei liegt **innerhalb** des bind-mounted `.claude/`-
  Ordners, es ist also nur **ein** Mount nötig. `~/.claude.json` hingegen
  enthält nur Feature-Flags + UserID, keine Credentials.
